...
\begin{itemize}
  \item \textbf{C}orrect~: le système propose une identité correspondant à celle indiquée dans la référence ;
  \item \textbf{S}ubstitution~: le système propose une identité différente de l'identité présente dans la référence ;
  \item \textbf{I}nsertion~: le système propose une identité alors que le locuteur n'est pas identifié dans la référence ;
...
La figure \ref{fig:resultats_tranter} donne les différents résultats en fonction du type de transcription utilisée (\emph{asr} : transcription automatique, \emph{aseg} : segmentation et classification automatiques) et de l'usage des concepts (dénommés \emph{Cat} dans la figure) ou non. 
