...
Si les deux n\oe{}uds fils d'un n\oe{}ud $T$ sont appelés $T_{oui}$ et $T_{non}$, et le pourcentage d'exemples de $T$, qui seront attribués par une question aux fils $T_{oui}$ et $T_{non}$, sont notés $p_{oui}$ et $p_{non}$ respectivement, la changement d'impureté $\Delta I$ est défini par la formule \ref{eq:gini}~:

\begin{equation} \label{eq:gini}
    \Delta I = I(T) - \frac{|T_{oui}|}{|T|} I (T_{oui}) - \frac{|T_{non}|}{|T|} I(T_{non})
\end{equation}

L'impureté d'un n\oe{}ud $t$ avec $i$ et $j$ recouvrant l'ensemble des classes est calculé avec la formule \ref{eq:impurity} :


où $p(i|t)$ est estimé avec la fréquence des exemples étiquetés avec la classe $i$ dans $t$.

La question associée à un n\oe{}ud $t$ sera la question qui maximise $\Delta I$

\subsubsection{Condition d'arrêt}

Le processus arrête de séparer un n\oe{}ud de l'arbre lorsque l'impureté d'un n\oe{}ud est sous un certain seuil ou lorsque le nombre d'exemples restant dans un n\oe{}ud est égal à 1.
