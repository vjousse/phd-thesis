...
L'évaluation a été réalisée sur le corpus d'apprentissage et sur un corpus de test de 10 heures de données en langue anglaise. Le comportement de chaque règle a été étudiée pour les noms de locuteurs présents dans l'enregistrement. Lors de l'évaluation, deux types de tours de parole ont été pris en compte. À savoir les tours de paroles \og purs \fg, c'est à dire les tours de paroles où seul un candidat est possible~: aucun conflit n'est présent dans ce type de tour de parole. À l'inverse, dans les tours de paroles \og impurs \fg, plusieurs noms complets sont en concurrence~: une des règle a donc attribué un nom complet de la transcription au mauvais tour de parole. Afin de calculer les erreurs commises par le système, différents cas de figure ont été pris en compte~:

\begin{itemize}
  \item C1 : l'identité extraite de la transcription est associée avec un tour de parole
  \item C2
  \item C3
  \item C4
  \item Undef
...
Sur le corpus d'apprentissage, les règles identifiant le locuteur courant sont les plus fiables (seulement 1,3\% de fausse identification), tandis que les règles identifiant les locuteurs précédents (25\% de fausse identification) et courants (12,5\% de fausse identification) présentent de taux d'erreur élevés. Cette tendance ce confirme sur le corpus de test avec 0,8\% d'erreurs pour les règles de type \og courant \fg, 16\% pour les règles de type \og suivant \fg et 34,2\% pour les règles de type \og précédent \fg. Le taux d'erreur global des règles sur le corpus de test est de 10,9\%. 

Avec un taux d'erreur des règles d'environ 11\%, ces travaux démontrent qu'identifier les locuteurs d'un document à partir de sa transcription est possible. En revanche, ils nécessitent un traitement manuel du corpus~: les règles et les dictionnaires de concepts sont réalisés par un humain. Le temps de mise en place de telles règles peut être long suivant la quantité de corpus à analyser et demande une expertise du domaine pour pouvoir être réalisée. Dans les travaux décrits dans \cite{CansecoRodriguez2005,CansecoRodriguez2006}, un corpus en langue anglaise de 150 heures a été étudié. De plus, le passage d'un corpus à un autre est fastidieux : il faut réécrire le jeu de règles pour l'utiliser sur des documents d'une autre langue ou provenant d'autres types d'émissions.

Ces travaux traitent du problème de l'attribution locale des étiquettes \og précédent \fg, \og courant \fg et suivant \og. L'attribution globale 
