...
Les arbres de classification sémantiques (SCT - Semantic Classification Tree) sont des arbres de décision binaire s'appuyant sur des patrons linguistiques comme paramètres de décision pour classer les échantillons. Dans le cadre de l'identification nommée, ils sont utilisés par le LIUM pour réaliser l'attribution locale des étiquettes \og{}précédent\fg{}, \og{}courant\fg{} et \og{}suivant\fg{} décrites dans la section \ref{ssec:attributions_locales}. Ces arbres sont activés sur le contexte lexical gauche et droit de chaque nom complet détecté par le système d'EN. Au maximum cinq mots à gauche et cinq mots à droite sont gardés pour l'apprentissage et l'utilisation de l'arbre. Le SCT va attribuer une probabilité à chaque étiquette possible pour un nom complet détecté dans la transcription. La figure \ref{fig:sct} donne un exemple d'arbre de classification utilisé par le LIUM. Les détails sur l'apprentissage de l'arbre sont décrits dans \cite{MauclairOdyssey06}.
...
Dans les travaux du LIUM, seule l'étiquette avec la plus grande probabilité est prise en compte, les autres probabilités sont ignorées. Lorsque deux décisions du SCT attribuent le même nom complet à une classe de locuteur anonyme, chacune des probabilités est additionnée. On obtient alors, pour chaque nom complet candidat pour nommer une classe anonyme, un score qui peut être utilisé pour prendre une décision.

Dans l'exemple de la figure \ref{fig:attrib_locale_lium}, le tour de parole correspondant à la classe de locuteur \emph{LOCU2} se voit affecter deux fois le nom complet \emph{Maude Bayeu}. Dans ce cas, le nom complet \emph{Maude Bayeu} aura pour score 1,22. À noter que par les sommes qu'il réalise, le LIUM ne peut par la suite plus parler de probabilités.
...
Lors du processus d'attribution globale, le nom complet avec le plus grand score pour une classe donnée est attribué à cette classe. Chaque classe de locuteur est traitée indépendamment de l'autre, c'est à dire que rien n'empêche le système d'attribuer le même nom de locuteur à deux classes différentes. Chose qui, si la segmentation et la classification ne sont pas remises en causes, ne devrait pas pouvoir arriver.

\subsection{Expériences et résultats}

Le système a été évalué sur des données provenant de la campagne d'évaluation des systèmes de transcriptions en français ESTER I \cite{Galliano05}. Ces données regroupent six radios différentes et correspondent à 81 heures d'enregistrements pour le corpus d'apprentissage (150 émissions), 12,5 heures pour le corpus de développement (26 émissions) et 10 heures pour le corpus de test (18 émissions). Le corpus de test a été enregistré 15 mois après les autres corpus et contient 2 radios qui ne sont pas présentes dans les corpus de développement et d'apprentissage. Toutes ces données ont été étiquetés en entités nommées grâce au système décrit plus haut.

La métrique utilisée pour évaluer ce système est la même que celle utilisée par Tranter dans \ref{Tranter}. Ce système a d'ailleurs été comparé à celui de Tranter dans \cite{Esteve07}. Il a été évalué sur des données segmentées et transcrites manuellement, et sur des données segmentées manuellement mais transcrites automatiquement. Ces transcriptions automatiques sont celles fournies par le LIMSI lors de la campagne ESTER I. Les résultats sont exprimés en terme de précision et de rappel dans les figures \ref{fig:results_yannick_manu} et \ref{fig:results_yannick_auto}. Le système de Tranter à base de N-grammes sert de comparaison.
...
Sur des transcriptions manuelles les deux systèmes font presque jeu égal. En revanche, sur des transcriptions automatiques, le système à base de SCT est clairement plus performant.
...
Cette thèse fait suite aux travaux déjà menés au Laboratoire d'Informatique de l'Université du Maine (LIUM) sur l'INL décrits dans \ref{sec:sys_id_lium}. En effet, dans \cite{Esteve07} il a été démontré que le système à base de SCT était plus performant sur des transcriptions automatiques que le système à base de N-grammes. Les travaux réalisés dans la présente thèse utilisent donc un SCT pour réaliser les attributions locales.

Nous commençons par décrire le système de détection des entités nommées, puis nous décrirons ensuite l'arbre de classification sémantique utilisé pour réaliser les attributions locales. Nous finirons par expliquer le processus d'attribution globale basé sur la théorie des fonctions de croyance avant de présenter les résultats du système.
...
Dans le but de réaliser un système d'INL fonctionnant sur des transcriptions réalisées par un SRAP, il faut tout d'abord disposer d'un système de détection des EN performant sur les transcriptions automatiques. La campagne d'évaluation Ester 2 \cite{Ester2} comprenait une tâche d'évaluation des systèmes de détection des entités nommées sur des transcriptions aussi bien manuelles qu'automatiques. Plusieurs laboratoires universitaires français ont participé à cette tâche comme le LINA, le LIMSI, le LIA, le LI (Tours) ou encore le LSIS. Deux entreprises privées ont aussi participé à l'évaluation~: Synapse et Xerox. La mesure utilisée pour l'évaluation est le Slot Error Rate (SER) décrit dans \cite{SER}.

Sur des transcriptions réalisées manuellement, Synapse et Xerox ont (de loin) les systèmes le plus performants avec un SER inférieur à 10~\%. Le troisième système est celui du LIA (LIA\_NE) qui obtient un SER de 23,9~\%. En revanche, sur des transcriptions réalisées automatiquement, c'est LIA\_NE qui arrive en première position avec un taux d'erreur de 43,4~\% sur les transcriptions du LIMSI qui affichent un taux d'erreur mot (WER) de 12,1~\% (soit le meilleur système de la campagne d'évaluation Ester 2). D'autres tests ont été réalisés avec des transcriptions contenant plus d'erreurs (17,83~\% et 26,09\% de WER), et dans tous les cas c'est LIA\_NE qui obtient les meilleurs performances. Les résultats sont présentés dans le tableau \ref{tab:ne_ester2}.
...
Afin de prendre en compte toutes ces spécificités LIA\_NE repose sur la collecte d'un maximum de connaissances (de manière non supervisée) sur les entrées du dictionnaire du SRAP. Pour ce faire, LIA\_NE utilise deux méthodes~:
\begin{itemize}
    \item Un processus d'extraction automatique des EN sur de larges corpus de textes. Ce processus utilise un étiqueteur en parties du discours associé à des données étiquetées manuellement. Ils vont permettre d'apprendre un modèle CRF \cite{CRFNE} (Conditional Random Field) pour ensuite étiqueter automatiquement un corpus de 1,3 giga mots.
    \item L'extraction automatique de données provenant de l'encyclopédie en ligne Wikipedia. Pour chaque entrée de Wikipedia, un graphe est généré représentant toutes les formes de ce mot présentes dans Wikipedia. Par exemple, pour \emph{Paris} 39 formes ont été extraites comme : \emph{Ville Lumière, Ville de Paris, Paname, Capitale de la France, Département de Paris,} etc. Ces différents mots et graphes extraits de Wikipedia sont ensuite associés à une des catégories d'EN de la campagne ESTER (personne, lieu, organisation, produit) grâce aux données d'apprentissage étiquetées manuellement et à des préalablement constitués au LIA.
\end{itemize}

LIA\_NE utilise tout d'abord un étiqueteur basé sur des HMM (Hidden Markov Model) qui permet d'étiqueter le document avec les parties du discours et des étiquettes sémantiques (personne, organisation, lieu, produit).
...
Là où les autres travaux utilisent des règles réalisées manuellement (cf. section \ref{sub:regles_manuelles}) ou des N-grammes (\ref{sub:regles_statistiques}), le LIUM utilise un type particulier d'arbre binaire, à savoir un arbre de classification sémantique.
...
À noter que le SCT ne choisit pas une étiquette parmi les quatre possibles, mais attribue une probabilité à chacune des étiquettes. Dans les travaux du LIUM, seule la probabilité la plus élevée est prise en compte. Cela permet de n'attribuer qu'une et une seul étiquette à un nom complet. En revanche, les informations fournies par les probabilités attribuées aux autres étiquettes ne sont pas prises en compte.

\section{Fonctions de croyances pour l'INL}


\subsection{Fonctions de croyances}

Dans cette section, quelques éléments de la th\'eorie des fonctions de croyances \cite{shafer76,smets94} sont bri\`evement rappelés. Dans cette thèse, l'interprétation propos\'ee par Smets est adopt\'ee~: le mod\`ele des croyances transf\'erables \cite{smets94}. Le but de ce mod\`ele est de d\'eterminer la croyance concernant diff\'erentes propositions, \`a partir d'un ensemble d'informations disponibles. Soit ${\cal E}$ un ensemble fini, appel\'e cadre de discernement de l'exp\'erience. La repr\'esentation de l'incertitude se fait gr\^ace au concept de la fonction de croyance, d\'efinie comme une fonction $m$ de $2^{\cal
E}$ dans $[0,1]$ telle que:
\begin{equation}
\sum_{A\subseteq {\cal E}} m(A)=1.
\end{equation} La quantit\'e
$m(A)$ repr\'esente la part de croyance allou\'ee exactement \`a la
proposition $A$. Les sous-ensembles $A $ de $\cal E$ tels que
$m(A)>0$ sont les {\em \'el\'ements focaux} de $m$.
%La quantit\'e $m(\emptyset)$, qui peut \^etre
%diff\'erente de $0$, repr\'esente la croyance allou\'ee \`a une
%proposition ext\'erieure \`a $\cal E$.
 Une structure de croyance est
\`a support simple si elle est focalis\'ee au maximum sur $\cal E$
et un seul sous-ensemble $A$. Elle est alors d\'efinie par:
\begin{equation}
\left\{\begin{array}{l}
m(A) = w, \quad w \in [0,1] \\
m({\cal E}) = 1-w
...
\right.
\end{equation}
L'ignorance totale est repr\'esent\'ee par la fonction de croyance
vide telle que $m(\Omega)=1$. Si tous les \'el\'ements focaux de $m$
sont des singletons, $m$ devient alors une mesure de probabilit\'e
(dite masse bay\'esienne).

La th\'eorie des croyances poss\`ede plusieurs outils d'agr\'egation
permettant de combiner des fonctions de croyance d\'efinies sur un
m\^eme cadre de discernement \cite{smets94}. En particulier, la
combinaison de deux structures $m_1$ et $m_2$ définies de fa\c con
``indépendante" sur $\cal E$ utilisant l'opérateur binaire
conjonctif non normalisé $\cap$ définit une fonction résultante $m'$
par la formule suivante \cite{smets94}:
\begin{equation} \label{eq:comb_Dempster}
\forall A\subseteq {\cal E}, \; m'(A)=\sum_{B \cap C =
A}m_1(B)m_2(C).
\end{equation}
On peut alors d\'efinir la combinaison de $n$ structures
$m_1,\ldots, m_n$ sur $\cal E$ par : $m= m_1 \cap \ldots \cap m_n$,
telle que
\begin{equation}\label{eq:combindetail}
m(A)= \sum_{A_1 \cap \ldots \cap A_n = A} \prod_{i=1}^{n} m_i(A_i)
\quad \forall A \subseteq \Omega.
\end{equation}
 Si on obtient une fonction de
croyance $m$ non normalisée (i. e. telle que $m(\emptyset)\neq 0$),
on peut la convertir par la proc\'edure de normalisation de Dempster
\cite{shafer76} en une structure normalis\'ee $m^*$ en
r\'epartissant proportionnellement la masse de l'ensemble vide sur
les autres éléments focaux :
...
%{\bf décision....}
Une fonction de croyance $m$ décrit l'état d'une croyance sur un
phénomène. Une fois qu'une structure $m$ est d\'efinie, il est
possible de la transformer en
 distribution de probabilit\'e, en particulier pour des aspects d\'ecisionnels.
  Une de ces distributions, appel\'ee
 probabilit\'e {\it pignistique}, consiste \`a r\'epartir \'equitablement la masse d'un sous-ensemble de $\Omega$ entre ses \'el\'ements.
 Cette distribution, not\'ee $P_{m}$, est d\'efinie pour tout $\omega \in \Omega$ par \cite{smets94}:
...
Comme décrit dans \ref{attributions_globales}, plusieurs occurrences correspondant à des noms complets différents peuvent être détectées. Plusieurs noms complets sont donc en compétition ou en conflit en tant que locuteur courant potentiel (ou précédent, ou suivant).
 ...
 On s'intéresse à un tour $t_k$ appartenant au locuteur $c_l$ et ayant $n_k$ occurrences. Soient $n_{k+r}$, le nombre
 d'occurrences tours précédent ($r=-1$) et suivant ($r=1$)
 de ce tour. Soient $\{o_{j,r}^k\},$
avec $ r=-1,0,1$ et $j=1,\ldots, n_{k+r}$, les occurrences de noms
complets détectés dans ces trois
 tours. Chaque occurrence $o_{j,r}^k$, correspondant à une étiquette $e_i$,
 représente une certaine information sur le locuteur du tour $t_k$
  qui peut être décrite par
 une masse de croyance $m^{jr}_{t_k}$ sur $\cal E$ à support simple, focalisée sur $e_i$ et $\cal E$:
 \begin{equation}\label{eq:putaindequationcruciale}
    ...
    \end{array}\right.
\end{equation}
où $\alpha_{il} \in [0,1]$ est une mesure de compatibilité de genre
entre $e_i$ et $c_l$. Si les genres sont connus avec certitude,
$\alpha_{il}=0$ si $g(e_i)\neq g(c_l)$ et $\alpha_{il}=1$ si
$g(e_i)= g(c_l)$. Si les prénoms sont ambigus (comme Dominique) ou
non spécifiés, ou si le genre du locuteur est incertain,
$\alpha_{il}\in ]0,1[$ est estimé à partir d'une base de prénoms et
...
\subsection{Combinaison par tour de parole et par locuteur}
 ...
 La première étape de la combinaison consiste à agréger les
 informations au sein d'un tour de parole donné.
La combinaison des $n_{k-1}+n_k+n_{k+1}$ masses ciblées sur le tour
$t_k$ et obtenues par l'équation \ref{eq:putaindequationcruciale}
peut se faire par la règle conjonctive
 de Dempster {\it non normalisée} (cf. équations \ref{eq:comb_Dempster}- \ref{eq:combindetail}), afin d'assurer l'associativité et la commutativité de la
 combinaison:
on obtient une masse de croyance $m_{t_k}$ sur l'identité du
locuteur du tour $t_k$, définie par
\begin{equation} \label{eq:massetour}
m_{t_k} =\displaystyle \bigcap_{r=-1}^{1}\bigcap_{j=1}^{n_{k+r}}
m^{jr}_{t_k}
\end{equation} et la masse
de croyance normalisée $m^*_{t_k}$ correspondante (cf. équation
\ref{eq:norm_Dempster}).

La deuxième étape de la combinaison consiste à agréger les résultats
obtenus par tour de parole pour l'ensemble de l'émission. Plusieurs
possibilités sont envisageables {\it a priori} pour combiner ces
informations. Il est possible de procéder de fa\c con  analogue à
celle de l'équation \ref{eq:sl}. Pour l'assignation d'un nom complet
$e_i$ à un locuteur $c_l$ donné, nous pourrions calculer un ``score"
pour chaque $e_i$, dénoté $sm_l(e_i)$, comme la somme des masses de
croyance concernant les tours de parole du locuteur $c_{l}$ :
\begin{equation}\label{eq:sml}
sm_{l}(e_i)= \sum_{t_k \in c_l } m^*_{t_k}(\{e_i\}).
\end{equation}
Les scores obtenus ne sont donc pas nécessairement normalisés (i.e.
$\leq 1$), ce qui rendra là encore difficile l'interprétation des
résultats lors du processus de décision. Nous préférons rester dans
le cadre de la théorie des croyances: une manière assez naturelle
 consiste en effet à continuer de combiner toutes les
masses de croyance relatives au même locuteur $c_l$ par la même
règle conjonctive de Dempster et donc à fusionner toutes les masses
de croyance relatives aux différents tours de parole $t_k$ de ce
locuteur. On obtient alors une masse de croyance globale $M_l$ sur
l'identité du locuteur $c_l$ de l'émission, définie par
\begin{equation} \label{eq:masseglobale}
M_{l} =\displaystyle \bigcap_{t_k \in c_l} m_{t_k}
\end{equation} et la masse
de croyance normalisée $M^*_{l}$ correspondante.


\subsection{Règle de décision}

La méthode exposée en section \ref{sec:decision} est utilisée comme base pour la décision d'affectation, mais celle-ci se trouve
simplifiée et unifiée grâce à l'utilisation des masses de croyance.
Les masses de croyance $M_l$ sont transformées en une probabilité
pignistique $P_{M_l}$ (cf. équation \ref{eq:pig}) et la règle $\mathbf{R}$ suivante est utilisée :
...
Ensuite, comme certains noms complets sont attribués lors de cette
première étape à plusieurs locuteurs différents, il faut là encore
réorganiser le partage des noms complets entre les locuteurs
anonymes. Le même processus de décision que celui
proposé en \ref{sec:decision} est appliqué, en rempla\c cant les scores
$\mbox{SC}_l$ par les probabilités pignistiques $P_{M_l}$. En
reprenant l'exemple proposé en \ref{sec:decision}, le nom complet
``Jacques Derrida" est là encore assigné aux départ aux trois
locuteurs $c_{13}$, $c_{14}$ et $c_{15}$ (cf. tableau
\ref{tab:examplecombdempster}). Mais finalement,``Jacques Derrida"
est également assigné à $c_{15}$, car c'est pour ce locuteur qu'il
obtient la plus grande probabilité pignistique.
%----------------------
\begin{table}[htbp]
...
Les attributions locales vont permettre d'affecter un nom complet à un tour de parole. Mais, comme mentionné dans la section \ref{attributions_globales}, des conflits peuvent apparaître. En effet, chaque tour de parole appartient à une classe de locuteur que l'on cherche à nommer. Tous les noms complets attribués à des tours de parole via la phase d'attribution locale sont des candidats potentiels pour nommer la classe anonyme correspondante. Nous décrivons ici le processus de décision utilisé par le LIUM pour résoudre ces problèmes.

\subsection{Notations}
$\mathcal{E}=\{e_1,\ldots, e_I\}$ correspond à l'ensemble des noms complets candidats pour nommer une classe. 

Soit $\mathcal{O}=\{o_1,\ldots, o_J\}$ les occurrences des noms complets étiquetés par le détecteur d'entités nommées dans les transcriptions.
$\mathcal{T}=\{t_1,\ldots, t_K\}$ désigne l'ensemble des tours de parole, et  $\mathcal{C}=\{c_1,\ldots, c_L\}$ l'ensemble des classes à nommer. Les traitements vont permettre d'attribuer un nom complet issu de $\mathcal{E}$ aux classes de $\mathcal{C}$.
Une classe est définie par $c_l~=~\{ t_k~\in~T /\ c_l\ est\ le\ locuteur\ de\ t_k \}$, chaque classe $c_l$ regroupant un ou plusieurs tours de parole $t_k$. Chaque tour de parole appartient à une et une seule classe.

%TAL Modification de probabilité en score probabiliste
Pour chaque occurrence d'un nom complet $o_j$ (pour $j= 1,\ldots, J$) détecté dans un tour de parole $t_k$, on notera  $t_{k-1}$ (respectivement $t_{k+1}$) le tour de parole précédant (respectivement suivant) celui où il a été détecté. De la même manière, le score $P(o_j,t_{r})$  désigne la probabilité que $o_j$ soit le locuteur du tour de parole $t_{r}$ avec $r \in \{k-1,k,k+1\}$. Ces scores sont fournis par l'arbre de classification, ils correspondent respectivement à l'étiquette \textit{\og tour précédent \fg }, \textit{\og tour courant \fg }, \textit{\og tour suivant \fg } et \textit{\og autre \fg }.


%La probabilité de l'étiquette \textit{\og autre \fg } n'intervient pas dans le processus de décision.

%SYL, il faut expliqué plus clairement comment la classe \og autre \fg  est utilisé, comme sa masse de probabilité agit
%SYL, ne faudrait il pas prendre une décision au max pour éliminer les cas \og autre \fg  contre (prev, next, curr)

%Comme l'a judicieusement fait remarquer christine, on s'en sert pas de ça ...
%Appelons $t_{k-1}(o_j)$ le tour de parole précédant celui où se situe $o_j$, $t_{k}(o_j)$ le tour de parole où se situe $o_j$ et $t_{k+1}(o_j)$ le tour de parole suivant celui où se situe $o_j$. Chaque classe de locuteur $c_l$ regroupe un ou plusieurs tours de parole $t_k$. Toutefois, chaque tour de parole appartient à une et une seule classe de locuteur.

\subsection{Calcul des scores}
\label{CalScore}

Pour l'assignation d'un nom complet $e_i$ à une classe $c_l$ donnée, nous calculons un score pour chaque nom complet $e_i$, dénoté $s_l(e_i)$, qui n'est autre que la somme des scores concernant les tours de parole de la classe $c_{l}$ :
...
Le LIUM fait l'hypothèse que la segmentation et le regroupement en classes sont corrects, ils ne sont donc pas remis en cause.
Le but est maintenant d'attribuer à chaque classe $c_l$ un nom complet $e_i$. 
%Soit $f: \mathcal{C}\rightarrow \mathcal{E}$ la fonction d'assignation des noms complets aux classes de locuteur.
...
\begin{itemize}
    \item tri des classes candidates pour un nom complet en fonction des scores;
    \item assignation du nom complet à la classe la plus probable.
\end{itemize}

Plusieurs stratégies peuvent être utilisées pour trier les classes $c_l$ en concurrence pour un nom complet $e_i$ donné. Prendre le score maximal $s_l(e_i)$ semble être la solution la plus naturelle. C'est celle qui a été choisis dans les travaux du LIUM.

Tous les noms complets possibles sont pris en compte {\it a priori} et triés en fonction de leur score $s_l(e_i)$. Premièrement, le nom complet avec le score maximal (noté $e_i^*$) est choisi, et si plusieurs classes sont associées au même $e_i^*$, alors ce nom complet sera assigné à la classe dont le score $s_l(e_i^*)$ est maximal. Ensuite, tous les noms complets choisis sont supprimés des classes qui n'ont pas encore été nommées. 

Un exemple concret est donné dans le tableau \ref{tab:examplecomb}. 
Le nom complet \og Jacques Derrida \fg{} a été assigné à trois classes différentes. Dans cet exemple, $c_{13}$ a le meilleur score et  \og Jacques Derrida \fg{} sera donc affecté à $c_{13}$. D'autres noms complets seront ensuite attribués aux classes $c_{14}$ et $c_{15}$.
%TAL suppression de la colonne avec les scores normalisés
\begin{table}[htbp]

\begin{center}
{\small
\begin{tabular}{|c c c|} \hline
    Classe & Nom complet $e_i^*$ &$s_l(e_i^*)$     \\ \hline
    $c_{13}$ &Jacques Derrida  &    $\mathbf{8,58}$               \\
    $c_{14}$   &Jacques Derrida &  1,67 \\
    $c_{15}$ &Jacques Derrida  & 4,94 \\
\hline
\end{tabular}
}
\end{center}
...
{\small
\begin{tabular}{|c c c |} \hline
     Classe & Nom complet $e_i^*$  (1\up{re} itération)&    2\up{e} itération      \\ \hline
    $c_{13}$ &\textbf{Jacques Derrida (8,58)} &  -             \\
    $c_{14}$   &Jacques Derrida (1,67) & \textbf{Alexandre Adler (0,30)} \\
    $c_{15}$ &Jacques Derrida (4,94) & \textbf{Nicolas Demorand (0,90)} \\
    $c_{16}$ &\textbf{Olivier Duhamel (0,93)} & -  \\
     % $c_{17}$ &\textbf{Marc Kravetz} (0.94) & Jacques Derrida  (0.33) \\
\hline
\end{tabular}
}
\end{center}
\caption{Exemple du processus de décision avec deux itérations (décision en gras, scores entre parenthèses).}
%\caption{}
...
{\begin{tabular}{|l|c|}
\cline{2-2}
  \multicolumn{1}{c|}{} &  \textbf{\textit{\'Evaluation}}\\ 
