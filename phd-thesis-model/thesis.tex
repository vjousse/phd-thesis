...
Si $T$ désigne un n\oe{}ud quelconque, et $f(j|T)$ le pourcentage d'exemples de ce n\oe{}ud qui appartiennent à la classe $j$, alors le critère d'impureté de Gini $i(T)$ est défini par~:

\begin{equation} \label{eq:impurity}
    i(T) = 1 - \sum_{j} f^2(j|T)
\end{equation}

Par exemple, prenons 3 classes $A$, $B$ et $C$. 10 exemples du corpus d'apprentissage arrivent dans un n\oe{}ud $T$. Sur ces 10 exemples, 4 appartiennent à la classe $A$, 3 à la classe $B$ et 2 à la classe $C$. Alors $f(A|T)=0,5$, $f(B|T)=0,3$ et $f(C|T)=0,2$. À partir de ces valeurs et de l'équation \ref{eq:impurity}, nous déduisons que le critère d'impureté de Gini $i(T)$ est égal à $0,62=1-(0,5^2 + 0,3^2 + 0,2^2)$.
...
\begin{equation} \label{eq:gini}
    \Delta I = i(T) - p_{oui} * i(T_{oui}) - p_{non} * i(T_{non})
\end{equation}
...
Dans notre cas, grâce à des expériences menées sur le corpus de développement, le seuil minimum du critère de Gini a été fixé à 0,01 et le nombre minimum d'exemples à 50.
...
Le SCT ne choisit pas une étiquette parmi les quatre possibles, mais attribue une probabilité à chacune des étiquettes. Une des possibilités retenue dans \cite{MauclairOdyssey06,Esteve07} est de ne retenir que l'étiquette qui a la plus forte probabilité. Le problème de cette approche est qu'elle occulte complètement les informations fournies par les probabilités des autres étiquettes.
...
Un des algorithmes parmi les plus utilisés pour résoudre ce problème est \og{} l'algorithme de Kuhn-Munkres \cite{MunkresJames} \fg{}, aussi appelé \og{} algorithme Hongrois \fg{}. Il consiste à trouver le chemin optimal qui minimise un certain coût. La fonction de coût utilisée est la suivante~:
