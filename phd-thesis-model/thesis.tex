...
Afin d'apprendre les caractéristiques de ce modèle CRF, un maximum de connaissances sur les entrées du dictionnaire du SRAP est collecté (de manière non supervisée). Ces connaissances seront utilisées lors de la phase d'apprentissage de LIA\_NE. Afin de réaliser ces collectes, deux méthodes sont mises en \oe{}uvre~:
\begin{itemize}
    \item Un processus d'extraction automatique des EN sur de larges corpus de textes. Ce processus utilise un étiqueteur en parties du discours associé à des données étiquetées manuellement en entités nommées. Ils vont permettre d'apprendre un modèle CRF pour ensuite étiqueter automatiquement un corpus de 1,3 giga mots. Ce corpus étiqueté automatiquement sera utilisé pour l'apprentissage du modèle CRF.
    \item L'extraction automatique de données provenant de l'encyclopédie en ligne Wikipedia. Pour chaque entrée de Wikipedia, un graphe est généré représentant toutes les formes du mot présentes dans Wikipedia. Par exemple, pour \emph{Paris}, 39 formes ont été extraites comme : \emph{Ville Lumière, Ville de Paris, Paname, Capitale de la France, Département de Paris,} etc. Chaque mot et ses différentes formes sont ensuite associées à une des catégories d'EN de la campagne ESTER (\emph{personne, lieu, organisation, produit}). \emph{Paris} (et ses différentes formes) sera par exemple associé à la catégorie \emph{lieu}. Cette association est réalisée grâce aux données d'apprentissage étiquetées manuellement et à des lexiques préalablement constitués au LIA. 
\end{itemize}
...
Les questions utilisées par les SCT pour classer les échantillons sont des expressions régulières. Les SCT apprennent ces expressions régulières à partir de corpus annotés avec les classes correspondantes. L'apprentissage et la sélection de ces questions est réalisée de manière entièrement automatique~: n'importe quel mot du corpus d'apprentissage peut apparaître dans une question. 
...
Afin de construire un arbre de classification sémantique, plusieurs éléments sont nécessaires~: un corpus d'apprentissage annoté, un ensemble de questions, un critère de séparation et une condition d'arrêt. Nous décrivons ces différents éléments ci-dessous.
...
L'arbre de classification sémantique apprend les expressions régulières grâce aux mots présents dans les contextes lexicaux gauche et droite de chaque nom complet détecté. Un maximum de 10 mots autour du nom complet détecté a été gardé~: au plus 5 mots à gauche et 5 mots à droite. Le nombre de mots à garder a été déterminé grâce au corpus de développement.
...
Le processus de construction du SCT doit choisir pour chaque n\oe{}ud l'expression régulière qui réduit au minimum un critère d'impureté. Ici le critère utilisé est le critère de Gini \cite{Breiman84}. La meilleure question (ici la meilleure expression régulière) est celle qui présente la plus grande chute d'\emph{impureté} entre le n\oe{}ud et ses enfants. Si les deux sous-arbres fils d'un n\oe{}ud $T$ sont appelés $T_{oui}$ et $T_{non}$ la chute d'impureté $\Delta I$ est définie par la formule \ref{eq:gini}~:
