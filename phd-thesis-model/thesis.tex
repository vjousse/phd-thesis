...
\begin{table}[h]
  \begin{center}
    ...
    \begin{tabular}{|c||c|c|c||c|c|}
      ...
      Évaluation & 89,33\% & 10,67\% & 0\% & 141 \\ \hline
      \hline
...
Les systèmes présentés dans le chapitre \ref{sec:inl} utilisent tous une liste de locuteurs potentiels connue à l'avance. Pour les résultats présentés dans cette section, une liste des locuteurs potentiels et de leur genre est aussi utilisée. Cela permet notamment de comparer le système à celui du LIUM présenté dans la section \ref{sec:sys_id_lium}. Les résultats sans cette liste de locuteur seront donnés et discutés dans la section \ref{}. \todo{Référence XP sans liste}

Le tableau \ref{tab:results_belief} présente les résultats du système sur les corpus de développement et de test, en utilisant des transcriptions totalement manuelles. La liste de locuteurs utilisée pour le corpus de développement correspond aux locuteurs du corpus d'apprentissage, de développement et de test de la campagne ESTER 1. La liste de locuteurs utilisée pour le test est celle du corpus de test EPAC. Les résultats en durée et en nombre de locuteurs sont donnés. 
\begin{table}[h]
\begin{center}
\begin{tabular}{|c|c|c|c|c|c|}
\cline{3-6}
...
\hline
\textbf{M} & \textbf{M} & 86,22\% & 94,41\% & 12,75\% & 15,23\% \\ \hline
\hline
\hline
\multicolumn{6}{|c|}{Corpus de Test} \\
\hline
\textbf{M} & \textbf{M} & 77,92\% & 95,48\% & 21,02\% & 24,53\% \\ \hline
\hline
\end{tabular}
\end{center}
...
Ces résultats sont comparables à ceux donnés dans la figure \ref{fig:results_yannick_manu}~: à 95~\% de précision, le meilleur système obtenait un rappel d'environ 40~\%. Le corpus utilisé dans \ref{fig:results_yannick_manu} correspond au corpus de développement du système présenté. Pour la même précision, le rappel a plus que doublé pour atteindre 86,22~\%. Sur le corpus de test, à 95~\% de précision, le rappel est d'environ 78~\%. 

Les moindres résultats sur le corpus de test s'expliquent par la composition du corpus décrit dans le tableau \ref{tab:donnees_xp}. En effet, il n'y a aucun locuteur étiqueté comme inconnu dans la référence. Le système doit donc proposer une identité pour chacun des locuteurs, quand bien même le locuteur a été nommé grâce à des informations externes au document, ou alors par un descriptif général comme \emph{un passant} ou \emph{un témoin}. Cela se traduit par un taux de locuteurs Indisponibles dans la traduction deux fois supérieur au corpus de développement.
