...
Nemesis a été développé par l'équipe TALN (Traitement Automatique des Langues Naturelles) du LINA (Laboratoire d'Informatique de Nantes Atlantique). À note qu'aucune modification ou adaptation n'a été réalisée pour utiliser Nemesis sur des transcriptions de journaux radiophoniques.
...
Cette étude a permis de remettre en question l'arbre de classification et notamment le processus de décision qui y est lié (cf. section \ref{sec:fct_croyances} sur le processus de décision de MILES\_NI). Les travaux intermédiaires menés sur ce processus de décision sont décrits dans le chapitre suivant. Nemesis a par la suite été remplacé par LIA\_NE, la différence de performances de MILES\_NI en utilisant Nemesis ou LIA\_NE est donnée dans le tableau X.
