...
Toutes les mauvaises décisions ont été étiquetées manuellement et comparées avec les différents scores générés par l'arbre. Cette étude a permis de mettre en évidence 
...
\textbf{(A1)} Les erreurs affectant le rappel proviennent majoritairement des locuteurs qui ont été étiquetés comme \textit{autres} alors qu'ils correspondent à une des 3 autres étiquettes. \\
``\texttt{\textit{Valérie Crova} dit : (...) qui a toujours entretenu selon [Jean Christophe Bouisson] des rapports tendus avec le gouvernement.
\\\textit{Jean Christophe Bouisson} dit : On ne peut pas penser (...)}''
\\Dans cet exemple ``[Jean Christophe Bouisson]'' est étiqueté comme \textit{autre} alors qu'il est le \textit{suivant}.
...
\textbf{(A2)} Les erreurs affectant principalement la précision sont dues à des locuteurs qui sont mal étiquetés avec les étiquettes \textit{suivant} et \textit{précédent}. Dans la majorité des cas, l'étiquette \textit{suivant} a été attribuée au lieu de \textit{autre}. Par exemple, l'annonce d'interview correspond à une erreur typique~:\\ 
``\texttt{[Jean Michel Hibon] au micro de [Jérôme Susini].}''\\
