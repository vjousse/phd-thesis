...
Toutes les règles précédemment apprises sont utilisées sur le corpus de test. Lorsqu'une règle avec une probabilité $p_1$ est déclenchée et suggère un nom complet $n_1$ pour un locuteur $s_a$, le score pour l'hypothèse $s_a = n_1$ est incrémenté.
...
Pour déterminer la probabilité d'attribuer un nom complet $n$ à un locuteur $s$, un ensemble de règles (N-grammes) applicables $\mathcal{R}(n)$ est déterminé. L'équation \ref{eq:sl} peut être généralisée de cette manière :

\begin{equation}\mathbf{}
  P(n|s) = 1 - \prod_{r\in \mathcal{R}(s)}(1-p_r)
\label{eq:sl_new}
\end{equation}
...
Pour attribuer un nom complet à un locuteur, il faut pouvoir choisir parmi les différents candidats possibles. Ce choix est effectué en prenant le nom complet candidat $n^*$ qui a la probabilité maximale pour le locuteur $s$ donné. Pour ce faire, la formule \ref{eq:decision_ngrammes} est utilisée~:
\begin{equation}\mathbf{}
  n^* = \operatorname{arg\,max}_n P(n|s) \mbox{ si } P(n^*|s) > \alpha
\label{eq:decision_ngrammes}
\end{equation}

Un seuil $\alpha$ est utilisé pour filtrer les décisions. Lorsque la probabilité est en dessous de la valeur fixée pour $\alpha$, aucune décision n'est prise pour $s$~: le locuteur est considéré comme inconnu.
