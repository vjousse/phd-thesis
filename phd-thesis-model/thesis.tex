...
Comme décrit dans le tableau \ref{tab:ne_ester2}, cette approche ayant obtenue les meilleurs résultats lors de la dernière campagne d'évaluation Ester 2, c'est tout naturellement qu'elle a été utilisée pour les travaux de cette thèse. Les 7 catégories d'entités nommées utilisées pour Ester 2 ont été gardées. Dans les transcriptions, chaque mot ou groupe de mot correspondant à une catégorie d'entités nommées a été remplacé par le nom de la catégorie. Nous discuterons dans X de l'apport de cette méthode pour l'identification nommée du locuteur, par rapport à une approche plus classique à base de règles manuelles prévues pour être utilisées sur des corpus écrits.
...
À noter que le SCT ne choisit pas une étiquette parmi les quatre possibles, mais attribue une probabilité à chacune des étiquettes. Dans les travaux présentés dans cette thèse, les probalités affectées à toutes les étiquettes sont prises en compte. En effet, nous estimons que même si une étiquette a une probalité supérieure aux autres, cela ne signifie pas nécessairement que c'est la bonne étiquette et que l'on ne doit prendre en compte que cette dernière. 
...
La section \ref{attributions_globales} décrit toutes les difficultés auxquelles la phase d'attribution globale a à faire face. Une de ces principales difficultés est de résoudre les conflits qui apparaissent à la suite de la phase d'atributions locales.
...
\subsection{Décisions globales : du maximum à la Théorie des croyances}
...
\subsubsection{Processus de décision}
