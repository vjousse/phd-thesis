...
Certains systèmes d'INL exploitent des connaissances a priori sur les locuteurs à identifier~: leur utilisation sur de grandes collections de données est difficile. En effet, obtenir un grand nombre d'informations a priori sur les locuteurs (comme des enregistrements de leur voix) est coûteux en temps. De plus, ces informations peuvent ne pas être disponibles. Idéalement, aucune connaissance a priori ne devrait être utilisée, de manière à pouvoir traiter un grand nombre de documents.

\todo{Introduire ça mieux}

L'hypothèse fondatrice de ces systèmes est que les locuteurs sont annoncés par leur prénom et leur nom et qu'il est possible d'exploiter cette information pour nommer les locuteurs du document. Le premier objectif des travaux présentés dans cette thèse est de disposer d'un système entièrement automatique, et plus particulièrement dans la phase de détection des noms complets dans la transcription. La détection des noms complets dans la transcription est une étape fondamentale du processus d'INL, et elle se doit d'être entièrement automatisée, et d'être performante sur des transcriptions issues de systèmes de reconnaissance automatique de la parole. 
Ensuite, il faut pouvoir utiliser ces noms complets détectés dans la transcription pour nommer les locuteurs. L'un des objectifs de cette thèse est de proposer une méthode permettant de prendre en compte les différents conflits qui peuvent apparaître à la suite de l'attribution locale.

Les travaux présentés dans cette thèse ont permis de mettre en \oe{}uvre un système de ce type. \emph{Milesin} est un système d'INL développé de manière à être le plus autonome possible~: il ne requiert pas d'informations a priori sur les locuteurs d'un document pour les identifier. Il utilise la transcription enrichie du signal audio pour nommer les locuteurs du document.


Cette thèse a été menée dans le cadre du projet régional \emph{MILES}. Elle est le fruit d'une collaboration entre le Laboratoire d'Informatique du Maine (LIUM) et le Laboratoire d'Informatique de Nantes Atlantique (LINA). Les travaux ont été menés sur des données en français, mais l'approche est applicable à d'autres langues.
...
Des premiers résultats sur des transcriptions entièrement automatiques ont été donnés. La majeure partie du travail réalisé pour \emph{Milesni} l'a été sur des transcriptions entièrement manuelles. De ce fait, le passage à l'automatique n'a pas encore été totalement approfondi et les performances sont moindres.
