...
L'identification nommée du locuteur (INL) consiste à attribuer un prénom et un patronyme (appelé \og nom complet \fg{}) à chaque locuteur d'un document audio. Les systèmes de reconnaissance de la parole récents s'intéressant aux transcriptions d'émissions radiophoniques (cf. \ref{ssec:trans_broadcast} fournissent une transcription dite enrichie (cf. \ref{sec:trans_enrichie}). Cette transcription contient notamment une segmentation et une classification en locuteurs (cf. \ref{ssub:decoupage_regroupement}) qui permet de délimiter les interventions des différents locuteurs dans l'enregistrement. En revanche, les informations fournies par le processus de segmentation et de classification en locuteur ne permettent pas d'identifier les locuteurs du document par leur prénom et nom. Les locuteurs sont uniquement identifiés par des labels anonymes du type \emph{Locuteur1}, \emph{Locuteur2}. L'INL cherche à remplacer ces labels anonymes par la vraie identité des locuteurs, à savoir par leurs noms complets (couple prénom/patronyme).
