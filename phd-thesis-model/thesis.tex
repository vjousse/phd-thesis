...
\section{Système de décision~: une modélisation à trouver}


\subsection{Utilisation d'un maximum}

Comme mentionné dans la section \ref{chap:liumni}, les travaux de cette thèse font suite à ceux précédemment menés au LIUM \cite{MauclairOdyssey06,Esteve07}. Le processus de décision utilisé dans ces travaux était assez rudimentaire.
...
Ce processus de décision a plusieurs inconvénients. Tout d'abord, le fait de réaliser la somme des probabilités issues de l'arbre ne permet plus de travailler dans l'espace probabiliste. En effet, une fois les sommes effectuées, seuls des scores difficilement comparables entre eux sont disponibles.


Ensuite, la règle $\mathbf{R_1}$ ne prend pas en compte le conflit au sein d'une classe de locuteur. Prenons l'exemple décrit dans le tableau \ref{tab:examplecomb}. En utilisant la règle $\mathbf{R_1}$, \emph{Jacques Derrida} est attribué à la classe de locuteur $c_{13}$ puisque c'est pour cette classe qu'il obtient le score maximum. $\beta_{il}$ désigne le pourcentage du score que représente $s_l(e_i^*)$ au niveau de la classe de locuteur $c_l$. Même si \emph{Jacques Derrida} a le score le plus élevé pour $c_{13}$, il ne représente que $35 \%$ des scores totaux de cette classe. Il y a donc un fort conflit au sein de $c_{13}$. En revanche, au sein de la classe $c_{15}$, \emph{Jacques Derrida} représente $80 \%$ des scores et semble beaucoup moins en conflit. Avant d'utiliser les fonctions de croyance, nous avons tout d'abord proposé une méthode permettant de prendre en compte ce conflit.
...
Comme décrit dans la section précédente, le conflit existant au sein des classes de locuteur peut être une information intéressante à prendre en compte lors du processus de décision. Nous proposons donc l'utilisation d'une nouvelle règle $\mathbf{R_2}$ formulée comme ceci~:
