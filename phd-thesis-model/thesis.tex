...
L'apport de cet article réside essentiellement dans le processus de
combinaison des différentes informations, en particulier des sorties
de l'arbre de classification.

\subsection{Fonctions de croyances}

Dans cette section, nous rappelons très bri\`evement quelques
éléments de la th\'eorie des fonctions de croyances
\cite{shafer76,smets94}. Dans cet article, nous adoptons
l'interprétation propos\'ee par Smets: le mod\`ele des croyances
transf\'erables \cite{smets94}. Le but de ce mod\`ele est de
d\'eterminer la croyance concernant diff\'erentes propositions, \`a
partir d'un ensemble d'informations disponibles. Soit ${\cal E}$ un
ensemble fini, appel\'e cadre de discernement de l'exp\'erience. La
repr\'esentation de l'incertitude se fait gr\^ace au concept de la
fonction de croyance, d\'efinie comme une fonction $m$ de $2^{\cal
...
Dans cet article, nous proposons d'améliorer le système proposé dans
\cite{ICASSP09, TAL09} en considérant la \emph{cohérence} des
informations au sein
 de tours de parole \emph{contigus}. En effet dans un \emph{même} tour, comme on l'a vu, plusieurs occurrences correspondant à des noms complets différents
peuvent être détectées. Plusieurs noms complets sont donc en
 compétition ou en conflit en tant que locuteur courant potentiel (ou précédent, ou suivant).
...
du corpus d'apprentissage. Le tableau \ref{tab:conflitdempster}
...
Pour la décision d'affectation, nous nous appuyons sur la méthode
exposée en section \ref{sec:decision}, mais celle-ci se trouve
simplifiée et unifiée grâce à l'utilisation des masses de croyance.
Nous transformons les masses de croyance $M_l$ en une probabilité
pignistique $P_{M_l}$ (cf. équation \ref{eq:pig}) et nous utilisons
la règle $\mathbf{R}$ suivante :
...
anonymes. On applique le même processus de décision que celui
proposé en \ref{sec:decision}, en rempla\c cant les scores
