...
Le taux d'erreur (en durée) obtenu varie de 65~\% à 81~\% en fonction du corpus utilisé. Lorsque l'on utilise \emph{Milesin} avec des transcriptions entièrement automatiques, les erreurs des différents systèmes se cumulent. Aucune adaptation de \emph{Milesin} pour des transcriptions automatiques n'a été effectuée dans ces expériences.

Une des premières choses à faire, serait d'essayer d'augmenter la précision comme décrit dans la section \ref{sec:perspectives_sans_liste}. Ensuite, une analyse des erreurs similaire à celle réalisée dans la section \ref{sec:analyse_preliminaire} devrait permettre de dégager de futures pistes de recherche. L'influence des différentes composantes d'une transcription automatique sur les résultats des systèmes d'INL est discutée dans la section \ref{sec:impact_trans_auto}.
...
La section \ref{} met en évidence les mauvaises performances de \emph{Milesin} lorsqu'il est utilisé sur des transcriptions entièrement automatiques.
...
\subsection{Impact de la transcription et de la segmentation}

\label{sec:impact_trans_auto}

Afin d'étudier l'impact des différentes composantes d'une transcription automatique (segmentation/classification et transcription), le tableau X montre les résultats en ne faisant varier qu'un des deux paramètres à la fois. Cette étude à été menée en utilisant une liste de locuteur par enregistrement.
