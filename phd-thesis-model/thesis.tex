...
Ces techniques nécessitent de détecter les différents noms complets cités dans la transcription pour ensuite les attribuer aux locuteurs du document. Il va donc falloir obtenir la transcription du signal audio pour pouvoir y détecter les différents noms complets qu'elle contient. Cette transcription devra aussi être enrichie avec une segmentation et une classification en locuteurs, puisque ce sont à ces locuteurs qu'il faudra attribuer les noms complets détectés dans la transcription. 
Comme le montre la figure \ref{fig:principe_inl}, la première étape consiste à réaliser une transcription enrichie du signal audio. Pour pouvoir être exploitée par les techniques d'identification nommée, cette transcription doit être préparée. La partie suivante décrit la préparation de cette transcription. 
