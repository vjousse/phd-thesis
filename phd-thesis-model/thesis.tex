...
En effet, comme décrit dans la partie précédente, chaque nom complet détecté dans un tour de parole est attribué ou tour de parole courant, précédent ou suivant. Chacun de ces tours de parole appartient à une classe de locuteur dite anonyme, que l'INL chercher à nommer en lui attribuant un et un seul nom complet. Or, plusieurs noms complets peuvent être candidats pour la même classe de locuteur. Comme le montre la figure \ref{fig:decisions_locales_conflit}, il est tout à fait possible que plusieurs noms complets différents soient en conflit au sein d'un même tour de parole. Dans l'exemple, deux noms complets sont possibles pour le tour de parole du locuteur numéro 2 : Maude Bayeu et Pierre Moscovici.
...
Chaque tour de parole appartenant à une même classe de locuteur, les conflits des tours de parole vont se répercuter au sein de la classe de locuteur. La figure \ref{fig:processus_entier} montre un exemple de décision avec conflits. 
Dans l'étape 1, la transcription est enrichie avec une segmentation et une classification en locuteurs ainsi qu'avec une détection des entités nommées. Chacune des entités nommées de type \emph{PERSONNE} sont ensuite attribuées localement au tour de parole correspondant dans l'étape 2. 

Par exemple, le nom complet \emph{C} est attribué aux tours de parole 5 et 11, qui correspondent respectivement à la classe du locuteur 3 et du locuteur 1. Le nom \emph{B} quant à lui se retrouve aussi en conflit, puisqu'il est attribué aux tours de parole 1 et 8 qui appartiennent respectivement à la classe du locuteur 1 et à la classe du locuteur 2.
...
La troisième étape consiste à résoudre ces conflits afin de ne plus avoir qu'un candidat par classe de locuteur pour pouvoir le répartir au sein du fichier entier dans l'étape 4.
...
L'attribution globale des noms complets est un problème complexe puisque nombre de conflits peuvent apparaître. Il appartient à chaque technique d'INL de résoudre ces conflits afin de nommer tous les locuteurs du document.
