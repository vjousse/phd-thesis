...
Tous les segments contigus appartenant au même locuteur définissent un tour de parole de ce locuteur. Un même locuteur qui parle à plusieurs endroits d'un enregistrement aura donc plusieurs tours de parole au sein de cet enregistrement. La technique utilisée pour regrouper les segments (et donc les tours de parole) par locuteur est une technique de classification automatique (cf. \ref{ssub:decoupage_regroupement}). On dit alors que les segments (et donc les tours de parole) d'un même locuteur sont regroupés au sein d'une même classe de locuteur. Dans les systèmes automatiques, ces classes de locuteurs sont dites anonymes. En effet, le nom de la classe n'est pas le vrai nom du locuteur, mais un nom unique choisi arbitrairement.
...
Les transcriptions, qui plus est les transcriptions de journaux radiophoniques, contiennent un grand nombre d'entités nommées (noms de personnes, lieux, radios, etc). Des systèmes de détection des entités nommées permettent de les repérer dans les transcriptions \ref{ssec:EN}. Ce sont des informations supplémentaires qui vont pouvoir être utilisées en recherche d'informations, ou plus précisément dans le cadre des présents travaux, pour identifier les les locuteurs d'un document.


\section{Architecture détaillée}

Les différentes campagnes d'évaluations des systèmes de RAP actuelles mettent l'accent sur la réalisation d'une transcription enrichie (cf. \ref{ssec:trans_broadcast}) via l'extraction d'informations sur les locuteurs ou encore la détection des entités nommées. La réalisation de cette transcription enrichie fait intervenir différents types de systèmes et de domaines de recherche. Afin de situer le contexte des travaux présentés dans cette thèse, il convient de détailler les différents axes de recherche qui entrent en jeu lors de la réalisation d'une transcription enrichie.

\subsection{Reconnaissance automatique du locuteur}
\label{ssec:ral}

La Reconnaissance Automatique du Locuteur (RAL) étant la première étape de la réalisation d'une transcription enrichie, c'est un élément fondamental des systèmes de reconnaissance automatique de la parole. Elle regroupe un ensemble de méthodes capables d'extraire les caractéristiques vocales propres à chaque individu à partir d'un document audio. 
  
Des serveurs vocaux aux terminaux mobiles en passant par les dispositifs de sécurité, la RAL est utilisée dans de nombreux domaines. Certains serveurs vocaux peuvent notamment l'utiliser pour détecter le genre du locuteur qui s'exprime. Par exemple, grâce à la plateforme MISTRAL \cite{MeignierMistral}, la société Calistel propose de router les appels provenant de locuteurs masculins vers des opératrices féminines et inversement, afin d'obtenir un taux de satisfaction des appels plus important. Mais l'application de la RAL la plus connue est certainement celle qui consiste à essayer d'identifier un individu par sa voix, pour lui autoriser l'accès ou non à une zone sécurisée, à savoir la biométrie. En effet, les systèmes de RAL vont essayer de reconnaître, un locuteur qui cherche à s'identifier au prêt d'un terminal. Contrairement à ce que l'on pourrait pense, même si ces systèmes donnent de bons résultats, ils ne sont pas aussi sûr que les empreintes digitales. Ils ne peuvent donc pas être employés, par exemple, à des fins judiciaires \cite{Bonastre2003}. En revanche, avec l'arrivée des terminaux mobiles dans la vie de tous les jours, la RAL s'invite avec succès dans les téléphones portables de dernière génération \cite{Larcher_JEP10}.
...
\subsection{Identification des locuteurs}

On va ici amener la partie suivante, sur l'ID nommée.
