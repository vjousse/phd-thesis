...
Les principales étapes et réflexions qui ont conduit au développement de \emph{Milesin}, et notamment une analyse des erreurs, sont tout d'abord présentées. Sont ensuite discuté la pertinence des métriques et de l'utilisation de connaissances a priori, avant de finir par l'utilisation de transcriptions automatiques.
...
Cette étude a permis de remettre en question l'arbre de classification ainsi que le processus de décision qui en découle (cf. section \ref{sec:fct_croyances} sur le processus de décision de \emph{Milesin}). Les travaux intermédiaires menés sur ce processus de décision sont décrits dans le chapitre suivant. Nemesis a par la suite été remplacé par LIA\_NE, la différence de performances de \emph{Milesin} en utilisant Nemesis ou LIA\_NE est donnée dans le tableau X.
\todo{Tableau perfs Nemesis}
%\subsection{Analyse des erreurs}
